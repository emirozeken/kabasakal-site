...
- [~] **Çalışma saatleri** — Google Haritalar işletme profilinden ("Kapanmak üzere · 00:00 · Açılış zamanı: Çar 12:00") günlük tam saat dökümü çekilemedi, `#konum`'a genel bir ifade yazıldı ("öğlenden gece yarısına kadar açık"); abiden gün gün kesin saatler alınınca hem `#konum` hem `#rezervasyon`'daki 12:00–22:00/30 dk saat aralığı (`js/main.js` → `reservationForm` → `buildTimeSlots`) gerçek saatlerle güncellenecek.
- [x] **Telefon numarası** — Google Haritalar işletme profilinden bulundu (0506 756 45 45), 2026-07-21. `#siparis`, `#konum` ve footer'daki `tel:+905067564545` / `https://wa.me/905067564545` linklerine işlendi. Paket servis platformu linki hâlâ TODO.
...
- [x] **Hikaye bölümü dükkan fotoğrafı** — `#hikaye` fotoğraf-perde bölümü: gerçek gece çekimi (`assets/hikaye/dukkan-gece.jpg/webp`, iyileştirilmiş) 100vh'lik `overflow:hidden` pencere içinde, JS (`hikayeCurtain`) her karede sahneye `translateY(-pencereninViewportTop'u)` vererek viewport'a çakılıymış gibi gösteriyor; scroll durmadan önceki/sonraki bölümler perde gibi üzerinden geçiyor (Sereno referansı). `background-attachment:fixed` (iOS'ta bozuk) ve `position:fixed`+`clip-path` (fixed'in ata clip'inden kaçması tarayıcıya göre değişiyor, sahne tüm sayfaya biniyordu — 2026-07-21'de canlıda yaşandı) bilinçli olarak KULLANILMIYOR. Daha fazla mekan çekimi gelirse aynı kalıpla ek perde bölümleri açılabilir.
